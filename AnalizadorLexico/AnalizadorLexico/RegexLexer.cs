...
namespace AnalizadorLexico
{
    class RegexLexer
    {
        ...
        public void AddTokenRule(string expresionR, string token_name, bool ignore = false)
        {
            if (string.IsNullOrWhiteSpace(token_name))
                throw new ArgumentException(string.Format("{0} no es un nombre válido.", token_name));

            if (string.IsNullOrEmpty(expresionR))
                throw new ArgumentException(string.Format("El patrón {0} no es válido.", expresionR));

            if (patron == null)
                patron = new StringBuilder(string.Format("(?<{0}>{1})", token_name, expresionR));
            else
                patron.Append(string.Format("|(?<{0}>{1})", token_name, expresionR));
